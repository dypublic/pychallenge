...
def myfun():
    ...
    while(i < last):
        if src[i].islower():
            #print("%d\n"%i)
            temp = src[i-3:i] + src[i+1:i+4]
            #if all(map(lambda x: x in string.ascii_uppercase, temp)):
            #for pos , res in enumerate( map(lambda x: x in string.ascii_uppercase, temp)):
            if any(map(lambda x: x.islower(), temp)):
                i += 3
                continue
            else:
                if i - 3 > 0 and src[i - 4] not in string.ascii_uppercase and \
                    i + 4 < len(src) and src[i + 4] not in string.ascii_uppercase :
        ...
        i += 1;
            ...
            #if src[i-3:i] in string.ascii_uppercase and src[i+1:i+4] in string.ascii_uppercase:
            #   result.append(src[i])
    print("".join(result))
    
    
code = src.replace("\n", "")
#code = "oooHHHoHHHHoHHHoHHHHjhgHHHoHHHiii"
def my1():
    word = ""
    i = 0
    last = len(code) - 8
    while i < last :
        skip = -1
        for pos, lower in enumerate(map(lambda x: x.islower(), code[i:i+7])):
            if (lower and pos != 3):
                skip = pos

        if skip == -1 :
            if code[i+7].isupper() or code[i-1].isupper() or code[i+3].isupper():
                skip = 7
            else :
                word += code[i+3]
                skip = 3
...
import re
def s2():
     word = "".join(re.findall('[^A-Z][A-Z]{3}([a-z])[A-Z]{3}[^A-Z]', src))
     #word ="".join(x[1] for x in re.findall('(^|[^A-Z])[A-Z]{3}([a-z])[A-Z]{3}([^A-Z]|$)', src))
     print(word)
...
if __name__ == '__main__':
    ...
    t0=Timer("myfun()","from __main__ import myfun")
    t1=Timer("my1()","from __main__ import my1")
    t2=Timer("s2()","from __main__ import s2")
    #t2=Timer("test2()",)
